...
function buildPrompt({rawText,targetRole}){
    return[
        ...
        : "No specific target role was provided -asess for thr role the candidate appears to be aiming for.",
        "",
        "Score the resume from 0-100 based on the ATS readiness (keyword match,parseable formatting,quantified impact,clarity).",
        "Return exactly 5 prioritized issues,5 standout strengths,and 5-10 week bbullets rwritten to be stronger,quantified and ATS-friendly.",
        "Rewrites must prserve the original meaning, Each rewrite needs a one line rationale.",
...
async function analyzeResume({ rawText, targetRole }) {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
        try {
            ...
            return {
                analysis: analysisValidator.parse(parsed),
                model: env.geminiModel,
